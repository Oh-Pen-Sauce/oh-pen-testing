...
import { getOhpenCwd } from "./ohpen-cwd";

export async function safeLoadConfig(): Promise<Config | null> {
  try {
    return await loadConfig(getOhpenCwd());
  } catch {
    return null;
  }
}
...
export async function listIssues(): Promise<Issue[]> {
  return sharedListIssues(getOhpenCwd());
}

export async function getIssue(id: string): Promise<Issue | null> {
  try {
    return await sharedReadIssue(getOhpenCwd(), id);
  } catch {
    return null;
  }
}
...
export async function updateIssue(issue: Issue): Promise<void> {
  await sharedWriteIssue(getOhpenCwd(), issue);
}

export async function listScans(): Promise<ScanRun[]> {
  const { scans } = ohpenPaths(getOhpenCwd());
  try {
...
export async function readSourceFileSlice(
  ...
  const abs = path.join(getOhpenCwd(), relPath);
